...
TEST(TestRange, last) {
    auto r = range{0, 10};
    ASSERT_EQ(r | last(), item(9));
    ASSERT_EQ(r | impl::next(), item(0));
}
...
TEST(TestRange, indices) {
    ...
    ASSERT_EQ(r | impl::next(), item(0ul));
    ASSERT_EQ(r | last(), item(std::numeric_limits<std::size_t>::max() - 1));
}

TEST(TestRange, inclusive) {
    auto r = inclusive_range{9, 10};
    ASSERT_EQ(r | impl::next(), item(9));
    ASSERT_EQ(r | impl::next(), item(10));
    ASSERT_FALSE(impl::next(r).has_value());
    ASSERT_FALSE(impl::next(r).has_value());
}
